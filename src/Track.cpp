...
bool Track::set_reg_context(REGDUMP_AVX512 regdump, bool eflags_zero_tf,
                            DWORD thread_id) {
  uc_err err;
    bool   bSuccess = true;
    ...
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM0,
                                 &regdump.regcontext.ZmmRegisters[0]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM1,
                                 &regdump.regcontext.ZmmRegisters[1]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM2,
                                 &regdump.regcontext.ZmmRegisters[2]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM3,
                                 &regdump.regcontext.ZmmRegisters[3]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM4,
                                 &regdump.regcontext.ZmmRegisters[4]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM5,
                                 &regdump.regcontext.ZmmRegisters[5]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM6,
                                 &regdump.regcontext.ZmmRegisters[6]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM7,
                                 &regdump.regcontext.ZmmRegisters[7]);
#ifdef _WIN64
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM8,
                                 &regdump.regcontext.ZmmRegisters[8]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM9,
                                 &regdump.regcontext.ZmmRegisters[9]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM10,
                                 &regdump.regcontext.ZmmRegisters[10]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM11,
                                 &regdump.regcontext.ZmmRegisters[11]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM12,
                                 &regdump.regcontext.ZmmRegisters[12]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM13,
                                 &regdump.regcontext.ZmmRegisters[13]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM14,
                                 &regdump.regcontext.ZmmRegisters[14]);
    err = m_VME.sim_uc_reg_write(UC_X86_REG_YMM15,
                                 &regdump.regcontext.ZmmRegisters[15]);
#else
#endif
    if (thread_id != 0) {
...
#ifdef _WIN64
            const uint64_t m_gdt_address = 0x0000C00000000000;
#else
...
bool Track::set_mem_context(DWORD process_id, DWORD thread_id) {
    ...
    if (hProcess != NULL && hThread != NULL) {
        ...
        if (m_handle_hook_mem_unmapped == NULL) {
          m_VME.sim_uc_hook_add(
              &m_handle_hook_mem_unmapped, UC_HOOK_MEM_UNMAPPED,
              reinterpret_cast<void *>(callback_event_mem_unmapped), this, 1,
              0);
        }
        return true;
    }
...
uc_err Track::start_track(TrackExitMsg *exit_msg) {
    ...
    if (m_handle_hook_code_execute == NULL)
      err = m_VME.sim_uc_hook_add(&m_handle_hook_code_execute, UC_HOOK_CODE,
                                  reinterpret_cast<void *>(callback_evnet_code),
                                  this, 1, 0);
